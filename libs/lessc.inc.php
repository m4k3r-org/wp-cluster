...
class lessc {
  ...
  //flawless_theme::console_log( $value[1], 'info' );
		$color = $this->coerceColor($value);
		if (is_null($color)) $this->throwError($error . ' for:  ' . $value[1] );
		return $color;
	}
	...
	function throwError($msg = 'Parse Error') {
		if (!empty($this->sourceParser)) {
			...
			if (isset($this->fileName)) {
				$loc = basename( $this->fileName ).' on line '.$line;
			} else {
				$loc = "line: ".$line;
			}
			...
			if ($this->peek("(.*?)(\n|$)", $m) && !empty($m[1]) ) {
				...
				throw new exception( $msg.': failed in '. $loc );
      }
		}
