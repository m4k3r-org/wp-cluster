...
if (!class_exists('cfct_module_callout')) {
	...
	/* Modified by potanin@UD */
      if (!empty($data[$this->get_field_name('button_text')])) {
				$button_text = esc_html($data[$this->get_field_name('button_text')]);
			}      
      
			return $this->load_view($data, compact('title', 'image', 'content', 'button_text', 'url'));
		}
	...
	/* Modified by potanin@UD */				
      if ($this->supports('button')) {
				$html .= '
						<div>
							<label for="'.$this->get_field_id('button_text').'">'.__('Button Text').'</label>
							<input type="text"  name="'.$this->get_field_name('button_text').'" id="'.$this->get_field_id('button_text').'" value="'.(!empty($data[$this->get_field_name('button_text')]) ? esc_html($data[$this->get_field_name('button_text')]) : '').'" />
						</div>';
			}
