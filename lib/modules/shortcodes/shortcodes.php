  ...
  namespace Flawless {
    ...
    class Shortcodes extends Module {
      ...
      public function __constructor() {
        ...
        add_action( 'flawless_content_type_added', array( $this, 'flawless_content_type_added' ) );
        add_action( 'flawless::init_lower', array( $this, 'init' ) );

        add_shortcode( 'code', array( __SELF__, 'code' ) );
        add_shortcode( 'breadcrumbs', array( __SELF__, 'breadcrumbs' ) );
        add_shortcode( 'current_year', array( __SELF__, 'current_year' ) );
        add_shortcode( 'site_description', array( __SELF__, 'site_description' ) );
        add_shortcode( 'post_link', array( __SELF__, 'post_link' ) );
        add_shortcode( 'button', array( __SELF__, 'button' ) );
        add_shortcode( 'get_permalink', array( __SELF__, 'get_permalink' ) );
        add_shortcode( 'image_url', array( __SELF__, 'image_url' ) );
        add_shortcode( 'pdf', array( __SELF__, 'google_docs_pdf' ) );
        add_shortcode( 'placeholder', array( __SELF__, 'placeholder' ) );
      ...
      public function init() {
        ...
        if ( !in_array( 'list-attachments', array_keys( $shortcode_tags ) ) ) {
          add_shortcode( 'list_attachments', array( __SELF__, 'list_attachments' ) );
        }
       ...
       * @version 1.25.0
       */
      static function placeholder( $args = array() ) {
       ...
       * @since 0.2.5
       */
      static function flawless_content_type_added( $args = false ) {
        global $flawless;
       ...
       * @author odokienko@UD
       */
      static function do_code_shortcode( $content ) {
        global $shortcode_tags;
       ...
       * @author odokienko@UD
       */
      static function flawless_stripslashes( $code ) {
       ...
       * @author odokienko@UD
       */
      static function code( $args = false, $content = null ) {

        wp_register_script( 'google-prettify', '//cdnjs.cloudflare.com/ajax/libs/prettify/r224/prettify.js', array( 'jquery' ), 'r298', true );
        wp_enqueue_style( 'google-prettify', '//cdnjs.cloudflare.com/ajax/libs/prettify/r224/prettify.css', array( 'flawless-app' ), Flawless_Version, 'screen' );

        /**
         * !important: if we want makes this static function working then we need to run "do_shortcode" twice.
        ...
        if ( empty( $args[ 'second_run' ] ) ) {
          ...
          $content = "<{$args['container']} class='{$args['class']}" . ( ( $args[ 'linenums' ] ) ? " linenums:{$args['linenums']}" : '' ) . ( ( $args[ 'lang' ] ) ? " " . $args[ 'lang' ] : '' ) . "'>" . Shortcodes::flawless_stripslashes( base64_decode( $content ) ) . "</{$args['container']}>";
       ...
       *
       */
      static function current_year( $atts = false ) {
        return date( 'Y' );
      }
       ...
       * @since 0.2.5
       */
      static function image_url( $attr, $content ) {
       ...
       *
       */
      static function google_docs_pdf( $attr, $content ) {
       ...
       *
       */
      static function site_description( $atts = false ) {
        return get_bloginfo( 'description' );
      }

      /**
       * Shortcode static function for getting a permalink to a specific post
       *
       * @since 0.1
       *
       ...
       * @return string
       */
      static function post_link( $attr ) {
      ...
      /**
       * Shortcode static function for getting buttons
       *
       ...
       * @return string
       */
      static function get_permalink( $attr ) {
