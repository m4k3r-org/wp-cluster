...
if (!class_exists('cfct_module_gallery') && class_exists('cfct_module_image')) {
	class cfct_module_gallery extends cfct_module_image {
        ...
        $gallery_atts = array(
					'id' => 0,
					'columns' => ($data[$this->get_field_name('columns')] ? $data[$this->get_field_name('columns')] : 3),
		...
		public function admin_form($data) {
          ...
          <li>
						<label for="'.$this->get_field_id('link_target').'_img">'.__('Columns', 'carrington-build').'</label>
						<input class="cfct-input-short" type="text" name="'.$this->get_field_id('columns').'" value="' . $data[$this->get_field_name('columns')] . '" id="'.$this->get_field_id('link_target').'_img"  />
					</li>
