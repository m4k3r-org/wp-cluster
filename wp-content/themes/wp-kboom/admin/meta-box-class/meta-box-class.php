...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_image( $field, $meta ) {
    $this->show_field_begin( $field, $meta );
    $html = wp_nonce_field( "at-update-mupload_{$field['id']}", "nonce-update-mupload_".$field['id'], false, false );
    if (is_array($meta)){
      if(isset($meta[0]) && is_array($meta[0]))
      $meta = $meta[0];
    }
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      $html .= "<span class='mupload_img_holder'><img src='".$meta['src']."' style='width: 99%;' /></span>";
      ...
      //$html .= "<input class='at-delete_image_button button-secondary' type='button' rel='".$field['id']."' value='Delete Image' />";
      $html .= "<input class='at-change_image_button button-secondary' type='button' rel='".$field['id']."' value='Change Image' />";
    }else{
      ...
      $html .= "<input class='at-upload_image_button button-secondary' type='button' rel='".$field['id']."' value='Upload Image' />";
    }
    echo $html;
  }
