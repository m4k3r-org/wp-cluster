jQuery( function( $ ) {
  ...
  $( "a.display-options" ).each( function() {
    var $displayOptionsButton = $( this ), $widget = $displayOptionsButton.closest( "div.widget" );
  ...
  $( "div#widgets-right" ).on( "click", "a.add-condition",function( e ) {
    e.preventDefault();
    var $condition = $( this ).closest( "div.condition" ), $conditionClone = $condition.clone().insertAfter( $condition );
    ...
    $conditionClone.find( "select.conditions-rule-minor" ).html( "" ).attr( "disabled" );
      ...
      if( $( this ).hasClass( 'active' ) )
        $widget.find( 'input[name=widget-conditions-visible]' ).val( '1' ); else
  ...
  $( "div#widgets-right" ).on( "click", "a.delete-condition",function( e ) {
    ...
    if( $condition.is( ":first-child" ) && $condition.is( ":last-child" ) ) {
      ...
      var $widget = $( this ).closest( "div.widget" ), $displayOptionsButton = $widget.find( "a.display-options" );
