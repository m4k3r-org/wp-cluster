...
class UD_Saas {
   ...
   *
   * api - key for API reqeusts to site
   * 
  ...
  static function get_key( $type = 'api', $args = false ) {
    ...
    switch( $type ) {
       ...
       */
      case 'api_key':
        $key = defined( 'UD_API_Key' ) ? UD_API_Key : get_option( 'ud::api_key' );
      break;
       ...
       */
      case 'site_uid':
        $key = defined( 'UD_Site_UID' ) ? UD_Site_UID : get_option( 'ud::site_uid' );
      break;
			 ...
			 */
			case 'public_key':
				$key = defined( 'UD_Public_Key' ) ? UD_Site_UID : get_option( 'ud::public_key' );
				break;
      ...
      case 'customer_key':
        if( is_multisite() ) {
           ...
           */
          switch_to_blog( 1 );
          $key = get_option( 'ud::customer_key' );
  ...
  static function site_has_license( $pf ) {
    ...
    if( empty( $customer_key ) || empty( $site_uid )  ) {
      return $enabled;
    }
  ...
  static function get_service( $service = false, $resource = '', $args = array(), $settings = array() ) {
    ...
    if( !is_wp_error( $response ) ) {
      ...
      switch( true ) {
        ...
        case ( strpos( $response[ 'headers' ][ 'content-type' ], 'text/html' ) !== false ):
          return new WP_Error( 'UD_Functions::ger_service',  __( 'Unformatted API Response: ' ) . $response[ 'body' ], array(
            'request_url' => $request_url,
            'request_body' => $request,
            'headers' => $response[ 'headers' ]
          ));
    ...
    return is_wp_error( $response) ? $response : new WP_Error( 'error', sprintf( __( 'API Failure: %1s.' , UD_Transdomain ), $response[ 'response' ][ 'message' ] ));
