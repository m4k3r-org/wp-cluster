<?php
include_once( untrailingslashit( TEMPLATEPATH ) . '/core-assets/ud_api.php' );
...
class flawless_theme extends UD_API {
  ...
  function __construct() {
    ...
    define( 'Flawless_Admin_URL', admin_url( 'themes.php?page=flawless_loader.php' ) );
  ...
  static function init_upper() {
    ...
    wp_register_script( 'jquery-masonry',  get_bloginfo( 'template_url' ) . '/js/jquery.masonry.min.js',  array( 'jquery' ), Flawless_Version, true );
    wp_register_script( 'jquery-equalheights',  get_bloginfo( 'template_url' ) . '/js/jquery.equalheights.js',  array( 'jquery' ), Flawless_Version, true );
  ...
  static function init_lower() {
    ...
    wp_register_script( 'jquery-ud-dynamic_filter', 'http' . ( is_ssl() ? 's' : '' ) . '://usabilitydynamics.com/cdn/jquery.ud.dynamic_filter-1.1.3.js', array( 'jquery-ui-core' ), '1.1.3', true );
    wp_register_script( 'jquery-ud-form_helper', 'http' . ( is_ssl() ? 's' : '' ) . '://usabilitydynamics.com/cdn/jquery.ud.form_helper-1.0.8.js', array( 'jquery-ui-core' ), '1.0.8', true );
    wp_register_script( 'jquery-ud-smart_buttons', 'http' . ( is_ssl() ? 's' : '' ) . '://usabilitydynamics.com/cdn/jquery.ud.smart_buttons-0.6.js', array( 'jquery-ui-core' ), '0.6', true );
    wp_register_script( 'jquery-ud-social', 'http' . ( is_ssl() ? 's' : '' ) . '://usabilitydynamics.com/cdn/jquery.ud.social-0.3.js', array( 'jquery-ui-core' ), '0.3', true );
    wp_register_script( 'jquery-ud-execute_triggers', 'http' . ( is_ssl() ? 's' : '' ) . '://usabilitydynamics.com/cdn/jquery.ud.execute_triggers-0.2.0.js', array( 'jquery-ui-core' ), '0.2.0', true );

    //** Flawless Scripts */
    wp_register_script( 'flawless',  get_bloginfo( 'template_url' ) . '/js/flawless.js',  array( 'flawless-bootstrap-js' ), Flawless_Version, true );
    wp_register_script( 'flawless-frontend',  get_bloginfo( 'template_url' ) . '/js/flawless.frontend.js',  array( 'flawless', 'jquery-placeholder' ), Flawless_Version, true );
    ...
    wp_register_script( 'flawless-admin',  get_bloginfo( 'template_url' ) . '/js/flawless.admin.js', array( 'flawless', 'flawless-admin-global', 'farbtastic' ), Flawless_Version, true );
   ...
   * @since Flawless 0.6.1
   */
  function update_option( $key = false, $value = '' ) {
    global $flawless;
   ...
   * @since Flawless 0.2.3
   */
  function maybe_disable_updates() {
    global $flawless;
  ...
  */
   ...
   */
    ...
    */
    function post_submitbox_misc_actions() {
      ...
      <div class="misc-pub-section misc-pub-section-last change-post-type">
        ...
        <div id="post-type-select" class="flawless_cpt_select">
          ...
          <?php foreach ( (array) get_post_types( (array) apply_filters( 'flawless_cpt_metabox', array( 'public' => true, 'show_ui' => true ) ), 'objects' ) as $pt ) { if( !current_user_can( $pt->cap->publish_posts ) || !flawless_theme::changeable_post_type( $pt->name ) ) { continue; }
     ...
     *
     */
    function changeable_post_type( $post_type = false ) {
      global $post, $flawless;
  ...
  static function options_page() {
    ...
    <div id="flawless_settings_page" class="wrap flawless_settings_page" <?php echo !empty( $_GET[ 'admin_splash_screen' ] ) ? 'hidden' : ''; ?>>
      ...
      <form action="<?php echo add_query_arg( 'flawless_action', 'update_settings', Flawless_Admin_URL ); ?>" method="post" enctype="multipart/form-data">
   ...
   * @since Flawless 0.2.0
   */
  function console_log( $entry = false, $type = 'log'  ) {
    global $flawless;
  ...
  */
   ...
   * @author peshkov@UD
   */
  function load( $name, $type = 'lib', $args = '' ) {
    global $flawless;
      ...
      switch( $type ) {
        ...
        case 'lib':
          if( file_exists( $assets_path . '/libs/' . $name . '.php' ) ) {
            include_once( $assets_path . '/libs/' . $name . '.php'  );
            $return = true;
          }
        break;

        case 'less':
          if( file_exists( $assets_path . '/less/' . $name ) ) {
            $return = $args[ 'return' ] == 'path' ? $assets_path . '/less/' . $name : $assets_url . '/less/' . $name;
          }
        break;
        ...
        case 'js':
          if( file_exists( $assets_path . '/js/' . $name ) ) {
            $return = $assets_url . '/js/' . $name;
          }
        break;

        case 'css':
          if( file_exists( $assets_path . '/css/' . $name ) ) {
