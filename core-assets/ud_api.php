...
class UD_API {
   ...
   *
   * api - the basic key the site issued when making it's first request, no validation is done
   * site_uid - a private key used by the client's WordPress application to identify itself to the UD API; for validation, only issued to sites accessible from the WWWW
   * customer_key - private keys issued to individuals directly, may be used on multiple sites and in most cases can take place of a missing site_uid
   *
   * To manually set your credentials:
   *
   * Only set your UD_Site_UID if you know what you're doing. The key should only be used on one website at a time to avoid conflicts.
   ...
   *
   * To set: define('UD_API_Key', 'some_made_up_key');
   * To set: define('UD_Site_UID', 'lPvY4kyfPUe8CcAh0ngF');
   * To get: WPP_F::get_key( 'site_uid' );
   *
   * @updated 1.36.0
   */
  static function get_key( $type = 'api', $args = false ) {
    ...
    switch( $type ) {
      ...
      /**
       * Legacy API key which will be going away eventually.
       */
      case 'legacy_api_key':
       ...
       */
      case 'api_key':
        if( !$response = defined( 'UD_API_Key' ) ? UD_API_Key : get_option( 'ud::api_key' ) ) {
          if( !is_wp_error( $response = wp_remote_get( 'http://api.usabilitydynamics.com/updates/key/' ) ) ) {
       ...
       * if a valid customer_key is passed.
       */
      case 'site_uid':
        if( !$response = defined( 'UD_Site_UID' ) ? UD_Site_UID : get_option( 'ud::site_uid' ) ) {
   ...
   *
   * @todo Add check to ensure passed variable isn't already encoded. - potanin@UD 9/12/12
   * @updated 1.0.4
   * @since 1.0.2
   * @author potanin@UD
   */
