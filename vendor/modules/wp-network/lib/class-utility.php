...
namespace UsabilityDynamics\Network {
  ...
  if( !class_exists( 'UsabilityDynamics\Network\Utility' ) ) {
    ...
    class Utility extends \UsabilityDynamics\Utility {
      ...
      function switch_to_network( $new_network = 0, $validate = false ) {
        ...
        if( ( true == $validate ) && !Utility::network_exists( $new_network ) )
          return false;
       ...
       * @param bool|string $site_name Name of the root blog to be created on the new
       *                           network
       * @param bool|int    $clone_network ID of network whose networkmeta values are
       *                                to be copied - default NULL
       * @param array|bool  $options_to_clone Override default network meta options to copy
      ...
      function add_network( $domain, $path, $site_name = false, $clone_network = false, $options_to_clone = false ) {
        ...
        if( !empty( $new_network_id ) ) {
          ...
          if( defined( 'SITE_ID_CURRENT_SITE' ) && Utility::network_exists( SITE_ID_CURRENT_SITE ) ) {
            Utility::switch_to_network( SITE_ID_CURRENT_SITE );
        ...
        if( !empty( $clone_network ) && Utility::network_exists( $clone_network ) ) {

          $options_cache = array();
          $clone_network = (int) $clone_network;

          Utility::switch_to_network( $clone_network );
          ...
          Utility::restore_current_network();
          Utility::switch_to_network( $new_network_id );
