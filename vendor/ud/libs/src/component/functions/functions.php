<?php
/*
 * This file is part of the UD package.
 *
 */
...
class Functions {
   ...
   *
   * <code>
   * UD\Functions::log( "Settings updated." );
   * </code>
   *
  ...
  static function _log( $message = false, $args = array() ) {
    ...
    if ( !did_action( 'init' ) ) {
      _doing_it_wrong( __FUNCTION__, sprintf( __( 'You cannot call UD\Functions::log() before the %1$s hook, since the current user is not yet known.' ), 'init' ), '3.4' );
    }
   ...
   *
   * $best_template = UD\Functions::get_template_part( array(
  ...
  static function add_log_page() {
    ...
    if ( did_action( 'admin_menu' ) ) {
      _doing_it_wrong( __FUNCTION__, sprintf( __( 'You cannot call UD\Functions::add_log_page() after the %1$s hook.' ), 'init' ), '3.4' );
      return false;
    }
