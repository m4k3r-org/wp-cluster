...
// row content output
	...
	public function admin(array $opts, array $data = array(), $template) {
		...
		/* Modified by potanin@UD so filter in row_html knows what row it is and can load custom classes */
		$html = $this->row_html(true, $opts); 
	...
	public function html(array $opts, array $data = array(), $template) {
		...
		$generated_row_classes = apply_filters(
			...
			$module_types,
			$this,
			$opts /* Modified by potanin@UD so filter knows the GUID of row */
		);
...
// templates
	 ...
	 *
	 * Modified by potanin@UD to pass the $opts argument
	 * @param bool $admin 
	 * @return string html
	 */
	public function row_html($admin = false, $opts = false) {
		if ($admin) {
			$html = '
		...
		return apply_filters('cfct-row-'.($admin ? 'admin-' : '').'html', 
			...
			$this->get_classes(),
			/* Modified by potanin@UD to pass row specific settings (such as GUID) to filter */
      $opts
		);
	}
