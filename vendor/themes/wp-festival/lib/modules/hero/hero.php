...
if ( ! class_exists( 'FestivalHeroModule' ) && class_exists( 'ImageModule' ) ) {
	class FestivalHeroModule extends ImageModule {
      ...
      ';
		 ...
		 *
		 * @param array $_data - saved module data
		 *
		...
		public function display( $_data ) {
			...
			/** Backup wp_query */
			$data = $wp_query->data[ 'hero' ] = array(
				'image_src'       => ( isset( $_data[ $this->get_field_name( 'image_id' ) ] ) ? wp_get_attachment_image_src( intval( $_data[ $this->get_field_name( 'image_id' ) ] ), esc_attr( $_data[ $this->get_field_name( 'image_id' ) . '-size' ] ), false ) : '' ),
				'image_alignment' => ( isset( $_data[ $this->get_field_name( 'image_id' ) ] ) ? esc_attr( str_replace( '-', ' ', $_data[ $this->get_field_name( 'hero_alignment' ) ] ) ) : '' ),
				'title'           => ( ! empty( $_data[ $this->get_field_name( 'title' ) ] ) ? esc_html( $_data[ $this->get_field_name( 'title' ) ] ) : '' ),
				'content'         => ( ! empty( $_data[ $this->get_field_name( 'content' ) ] ) ? $this->wp_formatting( $_data[ $this->get_field_name( 'content' ) ] ) : '' ),
				'box_height'      => ( ! empty( $_data[ $this->get_field_name( 'box-height' ) ] ) ? intval( $_data[ $this->get_field_name( 'box-height' ) ] ) : 0 ),
				'id_base'         => $this->id_base,
				'url'             => $this->get_link_url( $_data ),
				'fb_like'         => ( ! empty( $_data[ $this->get_field_name( 'fb_like' ) ] ) && $_data[ $this->get_field_name( 'fb_like' ) ] == 'true' ? true : false ),
				'fb_app_id'       => ( ! empty( $_data[ $this->get_field_name( 'fb_app_id' ) ] ) ? esc_html( $_data[ $this->get_field_name( 'fb_app_id' ) ] ) : '' ),
				'fb_url'          => ( ! empty( $_data[ $this->get_field_name( 'fb_url' ) ] ) ? esc_html( $_data[ $this->get_field_name( 'fb_url' ) ] ) : '' ),
				'tw_share'        => ( ! empty( $_data[ $this->get_field_name( 'tw_share' ) ] ) && $_data[ $this->get_field_name( 'tw_share' ) ] == 'true' ? true : false ),
				'tw_account'      => ( ! empty( $_data[ $this->get_field_name( 'tw_account' ) ] ) ? esc_html( $_data[ $this->get_field_name( 'tw_account' ) ] ) : '' ),
				'tw_hashtag'      => ( ! empty( $_data[ $this->get_field_name( 'tw_hashtag' ) ] ) ? esc_html( $_data[ $this->get_field_name( 'tw_hashtag' ) ] ) : '' ),
				'gp_share'        => ( ! empty( $_data[ $this->get_field_name( 'gp_share' ) ] ) && $_data[ $this->get_field_name( 'gp_share' ) ] == 'true' ? true : false ),
			);
