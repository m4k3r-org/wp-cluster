 ...
 * Application Loader
 *
 * @todo Make the baseUrl dynamically fetched from /admin-ajax.php?action=/wp-interstate/config or something.
 *
 */
define( 'app', [ 'module', 'require', 'exports' ], function( module, localRequire, exports ) {
  ...
  /** Pull in our config */
  require.config( {
    baseUrl: exports.baseUrl = document.getElementsByName( 'festival:url' )[0].content + '/static/scripts/src'
  } );
  
  /** Ok, pull in our config */
  require( [ 'components/require.config' ], function( config ){

    /** Ok, setup our config now */
    components.baseUrl = document.getElementsByName( 'festival:url' )[0].content + '/static/scripts/src';
    components.paths.jquery = 'components/jquery/jquery.min';
    require.config( components );
    
    /** Ok, now we can actually go through the rest of the app */
    require( [
        'module',
        'require',
        'exports',
      ...
      function( module, require, exports, $, developer, equalheights, swipe, countdown, ss, stickem, dotdotdot, stickynav, buytickets, navigation, masonry, carousel, account, streamFilter, artistProfile, collapse, share, imagelightbox, stream, fancybox, blogMain, tabbedContent, videoModule, fv ){
        ...
        if( doc.hasClass( 'page-home' ) ) {
          ...
          if( $( '.photos-videos-strip-container' ).length )
          {
            $( window ).on( 'resizeEnd', function() {
              ...
              } else if( !$( '.photos-videos-strip-container' ).data( 'initswipe' ) ) {
        ...
        else if( doc.hasClass( 'page-artist-lineup' ) ) {
          equalheights.equalize( $( '.main-artists .main-artist' ), 768 );
          equalheights.equalize( $( '.artist-lineup .callout, .artist-lineup.tier-one .main-artists' ), 768 );
          ...
          $( window ).on( 'resizeEnd', function() {
            ...
            } else if( !$( '.artist-slider-container' ).data( 'initswipe' ) ) {
        ...
        else if( doc.hasClass( 'page-features' ) ) {
          equalheights.equalize( $( '.features-content .feature-item' ), 768 );
      
        } // page-features
      
        else if( doc.hasClass( 'page-photo-gallery' ) ) {
          masonry.init( '.photo-grid' );
        } // page-photo-gallery
      
        else if( doc.hasClass( 'page-organizers' ) ) {
          equalheights.equalize( $( '.organizers-content .organizer-item' ), 768 );
        }
      
        else if( doc.hasClass( 'page-sponsors' ) ) {
        ...
        // Expose instance to global namespace as "app".
        window.app = window.app || module;
