...
require( [
  ...
  // Get the artist lineup photo width, so we know when to stop resizing
  if ( $('.artist-lineup-photo' ).length > 0 )
  {
    var artistLineupPhoto = $('.artist-lineup-photo img');

    var tmpImage = new Image();
    tmpImage.src = artistLineupPhoto.attr('src');

    $('.artist-lineup-photo' ).css({
      'max-width': tmpImage.width + 'px'
    });
  }
