...
// Messages
  ...
  cfct_messenger.setMessage = function( message, type, expire ) {
    ...
    $( this.opts.message_div ).attr( 'class', this.opts.message_type_classes[type || 'info'] ).html( '<span class="cfct-message-content">' + message + '</span>' );
      ...
      error: function( xhr, textStatus, e ) {
        switch( textStatus ) {
          case 'parsererror':
            ...
            var _html = '<p><b>Parse Error in data returned from server</b>' + ' <a href="#" onclick="cfct_builder.toggleAjaxErrorString(); return false">toggle</a></p>' + '<pre class="cfct-ajax-error-string" style="display: none;">' + _errstring.html() + '</pre>';
...
// Reordering Modules
  cfct_builder.initBlockSortables = function() {
    $( '.cfct-block-modules' ).each( function() {
      ...
      _this.sortable( $.extend( cfct_builder.opts.sortableDefaults, {
        remove: function() {
  ...
  cfct_builder.disableSortables = function() {
    ...
    $( '#cfct-sortables, .cfct-popup .multi-module-form' ).append( $( '<div class="cfct-reorder-status">' ).append( $( '<div class="cfct-reorder-status-wrapper" />' ).css( {
...
// Sideload Module functions
  ...
  cfct_builder.sideloadOpen = function( callback ) {
    ...
    // display
    sideloadr.css( {'height': popup_height, 'width': popup_width, 'left': -popup_width} ).find( '.cfct-popup-content' ).end().animate( {left: 0}, 'fast', callback || function() {
    } );
  };
...
// Module Callbacks
  cfct_builder.doModuleSaveCallback = function( form ) {
    ...
    if( cfct_builder.hasModuleSaveCallback( _form.attr( 'name' ) ) ) {
      $.each( cfct_builder.opts.module_save_callbacks[_form.attr( 'name' )], function( i, func ) {
        try {
  ...
  cfct_builder.doModuleLoadCallback = function( form ) {
    ...
    if( _form.attr( 'name' ) in cfct_builder.opts.module_load_callbacks ) {
      $.each( cfct_builder.opts.module_load_callbacks[_form.attr( 'name' )], function( i, func ) {
        try {
    ...
    if( '*' in cfct_builder.opts.module_load_callbacks ) {
      $.each( cfct_builder.opts.module_load_callbacks['*'], function( i, func ) {
        try {
          func.call( null, _form );
        } catch( e ) {
          if( window.console && console.log ) {
...
// Actions
  ...
  cfct_builder.bindLiveClickables = function() {
    ...
    if( !this.$rowTrigger.data( 'popover' ) ) {
      ...
      if( 'function' === typeof this.$rowTrigger.popover ) {
    ...
    $( 'a.cfct-module-toggle-render' ).live( 'click', function() {
      ...
      cfct_builder.fetch( 'toggle_render', module_data, 'toggle-render-response' );
      return false;
    } );
    ...
    $( '.cfct-global-image-select .cfct-image-select-items-list-item' ).live( 'click', function() {
      ...
      $( '.cfct-global-image-search-current-image .cfct-image-select-items-list-item > div', _wrapper ).css( 'backgroundImage', _this.children( ':first' ).css( 'backgroundImage' ) ).children( ':first' ).text( _this.children( ':first' ).children( ':first' ).text() );
...
// init post handler - trigger autosave and continue when post_ID has been updated
  cfct_builder.initPost = function( callback, data ) {
    ...
    setTimeout( function() {
      cfct_builder.continueInitPost( callback, data );
    }, 500 );
  };
...
// Utility
  ...
  cfct_builder.module_spinner = function( module, message ) {
    $( module ).append( $( '<div class="cfct-module-status">' ).append( $( '<div class="cfct-module-status-wrapper" />' ).css( {
...
// Get started
  $( function() {
    ...
    var url = location.href, urlParts = null, moduleId = '';
    if( url.indexOf( '#' ) != -1 ) {
      urlParts = url.split( '#' );
      moduleId = urlParts[1]
