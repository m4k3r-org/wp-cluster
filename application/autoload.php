...
if ( ! defined( 'WP_VENDOR_DIR' ) ) {
	define( 'WP_VENDOR_DIR', ABSPATH . 'wp-vendor/' );
}

if ( ! defined( 'WP_VENDOR_AUTOLOAD_PATH' ) ) {
	define( 'WP_VENDOR_AUTOLOAD_PATH', WP_VENDOR_DIR . '/autoload.php' );
}
...
if ( ! is_file( WP_VENDOR_AUTOLOAD_PATH ) ) {
	...
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG && function_exists( 'passthru' ) ) {
		...
		passthru( "composer.phar install -d '" . WP_VENDOR_DIR . "' -n --prefer-dist --no-dev --optimize-autoloader 2>&1", $return_code );
